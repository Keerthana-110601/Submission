...
namespace BankCustomerAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        // ---------------- NEW: Get accounts by userId ---------------- //

        [HttpGet]
        [Authorize(Roles = "Admin,BankUser,NormalUser")]
        public async Task<IActionResult> GetAccountsByUserId([FromQuery] int userId)
        {
            if (userId <= 0)
            ...
            try
            {
                ...
                if (accounts == null || accounts.Count == 0)
                    return NotFound(new { error = "No accounts found for this user" });
        ...
        public async Task<IActionResult> GetTransactions([FromQuery] int[] accountIds)
        {
